...
A Rime-based Wubi input schema for the Khitan Small Script

![契丹小字研究概况](images/image1.png)
...
​	契丹小字基本算是一种音节文字，加以一些表意文字，类似谚文。其基础原字总数约三百多字，其中仍有不少没有破译。在结构上，契丹小字参考汉字字形而创制，与汉字结构极相似，因此基本能够完美适配五笔输入法。

​	目前，契丹小字的输入法已有[仓颉输入法、笔画输入法](http://www.ccamc.co/fonts_kht_jrc.php)。加上本五笔方案，共有三种输入方案可供用户选择。
...
1. 下载并安装契丹小字字体，如[CCAMC KSS Kaiti](http://www.ccamc.co/fonts_kht_jrc.php)。
2. 下载[Rime输入法内核](https://rime.im/)并安装。
3. 右键Rime状态栏，选择并打开“用户文件夹”，将本项目的.yaml和.ico文件复制到该目录。
4. 右键Rime状态栏，选择“输入法设置”，并勾选“契丹小字五笔”。
5. 右键Rime状态栏，选择“重新部署”。
6. 按下ctrl+~，选择“契丹小字五笔”。
...
​	据统计，全部的482个字符中，有24组48字的两码均重，但可依靠取码顺序强行区分。另有23组68字无法区分（除去部首时为22组61字）。
